 ...
 */
export function extractIccFromHeic(data) {
 ...
 */
export function bitDepthFromHeic(data) {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    ...
    for (let off = ipco.payloadStart; off < ipco.end;) {
        ...
        if (box.type === BOX_PIXI && box.end - p >= 6) {
            ...
            for (let i = 0; i < num && p + 5 + i < box.end; i++)
                depth = Math.max(depth, view.getUint8(p + 5 + i));
        }
 ...
 */
function primaryProps(view, length) {
    const meta = findBox(view, 0, length, BOX_META);
    if (!meta)
        return [];
    ...
    if (!pitm || !ipma)
        return all; // single-image: no association table needed
 ...
 * data.
 */
export function injectIccIntoJpeg(jpeg, iccProfile) {
...
function buildIccMarkers(profile) {
