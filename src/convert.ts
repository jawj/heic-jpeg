...
class MozJPEGWriter {
  ...
  private readonly rowBufPtr: number;
  private readonly rowBytes: number;

  constructor(
    private readonly moz: MozJPEG,
    width: number,
    height: number,
  ) {
    moz.onImgChunk = (startPtr: number, length: number) => {
  ...
  writeScanline(src: Uint8Array, srcOffset: number): void {
...
export async function heicToJpeg(
  ...
  try {
    ...
    const handle = unwrapOrThrow(
      heif.heif_js_context_get_primary_image_handle(ctx),
    );
    try {
...
export async function heicToJpegAll(
  input: Uint8Array | ArrayBuffer,
  options: ConvertOptions = {},
): Promise<ConvertResult[]> {
  const inputData = asUint8Array(input);
  ...
  try {
    ...
    for (const id of ids as number[]) {
      const handle = unwrapOrThrow(
        heif.heif_js_context_get_image_handle(ctx, id),
      );
      try {
...
function readContext(heif: any, ctx: unknown, data: Uint8Array): void {
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function unwrapOrThrow(result: any): any {
